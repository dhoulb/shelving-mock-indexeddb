	...
	class IDBTransaction extends EventTarget
	{
		...
		constructor(db, storeNames, mode = 'readonly')
		{
			...
			function request(input, callback)
			{
				...
				if (aborted) throw new DOMException('IDBTransaction: Cannot create request when transaction has been aborted', 'InvalidStateError');
	...
	class IDBObjectStore
	{
		...
		constructor(transaction, storeName)
		{
			...
			function openCursor(query = undefined, direction = 'next')
			{
				...
				// Check params.
				if (!validKey(query) && !validKeyRange(query) && query !== undefined && query !== null) throw new DOMException('count(): The query parameter did not contain a valid key (number, string, date), key range (IDBKeyRange or array of valid keys), null, or undefined', 'DataError');
		...
		constructor(request, range = undefined, direction = 'next', withValue = false)
		{
			...
			function progress()
			{
				...
				if (keys.length)
				{
					...
					if (withValue) value = clone(transaction._data[storeName].records.get(primaryKey));
				}
				else
				{
					key = undefined;
